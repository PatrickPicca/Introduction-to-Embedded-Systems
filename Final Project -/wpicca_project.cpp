 ...
 *
 * Demo Link: https://youtu.be/9i-R8oh0KO0
 */
...
int TickFct_display(int state) 
{
  ...
  switch(state){ // Action Stat
    ...
    case generateLock:
      ...
      lockValue = random(2,5) + 1;    //values 1-5
      if (tumbleAt < lockValue)
 ...
 //   Serial.print(lockValue);
    ...
    case play:
      ...
      if (digitalRead(A0) == HIGH && buttonPress == false)
        {
            if (tumbleAt > 0)
              tumbleAt--;
            distance =  lockValue - tumbleAt;
      ...
      else if (digitalRead(A1) == HIGH && buttonPress == false)
        {
            if (tumbleAt < 7)
              tumbleAt++;
            distance =  lockValue - tumbleAt;
